			...
			if(args.length >= 2) {
				...
				if(t != null) {
					if(FriendManager.isFriend(p.getUniqueId(), t.getUniqueId())) {
						if(SettingsManager.isUsingFriendChat(t.getUniqueId())) {
							String msg = "";
							...
							p.sendMessage(Utils.getAsBaseComponent(FriendMessageUtils.prefix + cfg.getString("Messages.Friend.NotUsingFriendChat").replace("%player%", t.getDisplayName())));
						}
					} else {
				...
				p.sendMessage(Utils.getAsBaseComponent(FriendMessageUtils.prefix + cfg.getString("Messages.Friend.Help.MSG")));
			}
		} else {
